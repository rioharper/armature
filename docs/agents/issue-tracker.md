...
Issues and specs for this repo live as GitHub issues on `rioharper/armature`. Use the `gh` CLI for all operations. (`gh` infers the repo from the clone; on this dev machine it lives at `C:\Program Files\GitHub CLI\gh.exe` when not on PATH.)
...
- **Map**: a single issue labelled `wayfinder:map`, holding the Notes / Decisions-so-far / Fog body. `gh issue create --label wayfinder:map`. Find the active map with `gh issue list --label wayfinder:map --state open`.
