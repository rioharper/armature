...
Read the spec first. Usually it's a markdown file produced by the **robotics-spec-design** skill; it may also be uploaded or pasted. If no spec exists, don't fabricate one — do a compressed requirements capture (mission, constraints, chosen architecture, builder capability) and note in the plan that it rests on an informal spec, or suggest running robotics-spec-design first if the project is substantial. Audience and differentiation should already be settled further upstream, in **robotics-concept-design**'s concept brief if one exists — this skill takes that as given rather than re-interrogating it.
...
- **Symbol table:** q for joint positions, τ for torques, m_i, l_i, I_i for link properties, etc., with units. The **robotics-mathematician** skill will consume this table verbatim, so make it complete.
...
1. **Analysis & sizing** — kinematic model, workspace check, actuator sizing from dynamics, DOF/reachability verification. Tasks here should explicitly say "derive FK/IK/Jacobian/dynamics" — and if the **robotics-mathematician** skill is installed, name it as the tool for those tasks.
...
- Kinematics/dynamics tasks → **robotics-mathematician**
- "I'm stuck, need a better approach for phase N" → **robotics-inventor**
- "Explain this concept the plan assumes I know" → **robotics-teacher**
- "Stress-test this plan (or the spec under it) for gaps before I commit" → **robotics-red-team**
...
- **→ robotics-mathematician** (the common one, for a Phase-1 analysis/sizing task): "Run robotics-mathematician for `<task id, e.g. T1.2 — actuator sizing>` in the attached plan for `<project>`. Use the frames, symbol table, and naming conventions in the plan's Section 1 verbatim — that glossary is the contract. Goal: `<what this task must produce>`. Relevant parts and their limits are in the attached BOM."
- **→ robotics-red-team:** "Red-team the attached plan for `<project>` (and the spec under it, attached). Check phase sequencing, that every Must REQ traces to a verification task, and that each high risk has a prototype or decision point. Hours/week and deadline the plan assumes: `<…>`."
